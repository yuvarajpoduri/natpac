...
const seedTrips = async () => {
  try {
    ...
    if (!user) {
      console.log('No user found, creating a dummy presentation user...');
      user = await User.create({
        fullName: 'Presentation User',
        emailAddress: 'presentation@natpac.gov.in',
        passwordHash: '$2b$10$K9rB.K3fL9uB9KjR7W4yO.uW7vF9X3vK4zL5mN6oP7qR8sT9uV1wX', // dummy hash
        userRole: 'citizen',
        points: 500,
        consentGiven: true
      });
    ...
    console.log(`Adding trips to user: ${user.fullName} (${user.emailAddress})`);

    // 2. Clear existing trips for this user (optional, but requested for presentation)
    ...
    for (let i = 0; i < 13; i++) {
      ...
      tripDate.setDate(now.getDate() - Math.floor(i / 2)); // Spread over last 7 days
      tripDate.setHours(8 + (i % 3) * 4, Math.floor(Math.random() * 60));

      const durationSeconds = 900 + Math.random() * 3600; // 15 mins to 1.25 hours
      const distanceMetres = 2000 + Math.random() * 15000; // 2km to 17km
      const avgSpeed = (distanceMetres / durationSeconds) * 3.6; // km/h
